...
const io = require("socket.io")(server, {
	cors: {
		origin: "http://localhost:3000", // Sesuaikan dengan URL domain frontend Anda
		methods: ["GET", "POST"], // Metode yang diizinkan
		allowedHeaders: ["my-custom-header"],
		credentials: true, // Jika menggunakan cookies atau otentikasi
	},
});
...
async function connectToWhatsApp() {
	const { state, saveCreds } = await useMultiFileAuthState("session_user_one");
	...
	sock.ev.on("connection.update", async (update) => {
		...
		if (qr) {
			...
			this.qr = qrCode; 
			soket?.emit("qr", qrCode); // Assuming your socket instance is stored in sock
			soket?.emit("log", "QR Code received, please scan!"); // Emit a log message to the client
		}
		if (connection === "close") {
...
const updateQR = (data) => {
	switch (data) {
		case "qr":
			...
			break;
		case "connected":
			soket?.emit("qrstatus", "/assets/svg/check.svg");
		...
		case "qrscanned":
			...
			console.log("QR Code Telah discan!");
			break;
		case "loading":
			soket?.emit("qrstatus", "/assets/svg/loader.gif");
			soket?.emit("log", "Registering QR Code , please wait!");
			console.log("Registering QR Code , please wait!");
			break;
		default:
			break;
	}
...
// delete session
app.post("/hapus-session", async (req, res) => {
	try {
		// Path ke folder session
		const sessionFolderPath = path.join(__dirname, "session_user_one");
		...
		if (sock) {
			console.log('sock ada');
			await sock.ev.removeAllListeners("connection.update");
			sock = null;
		}

		await connectToWhatsApp();
		res.status(200).json({ message: "Session deleted and QR Code generated" });
	} catch (error) {
		console.error(error);
		res.status(500).json({
			error: "Terjadi kesalahan saat menghapus isi file dalam folder session.",
		});
	}
});
